...
struct StoryBrain {
    ...
    mutating func checkAnswers(sender: UIButton?) {
    ...
    private func getStoryText() -> String {
        let story = stories[storyNumber].title
        return story
    }
    
    func updateCurrentStory() {
        views.storyLabel.text = getStoryText()
        views.choice1Button.setTitle(stories[storyNumber].choice1, for: .normal)
        views.choice2Button.setTitle(stories[storyNumber].choice2, for: .normal)
    }
}
